 ...
 *
 * DOM-layer round-trip suite. Every other test exercises pure-function
 * paths (`spec.toComark` / `spec.fromComark` and the orchestrator's
 * helpers); none of them go through Tiptap's `parseHTML` / `renderHTML`
 * / `mergeAttributes`. That gap let the [comment.ts] payload-loss bug
 * ship in the first place — a non-empty comment text was being clobbered
 * by `mergeAttributes(HTMLAttributes, { 'data-comark-comment': '' })`,
 * which had no specification-level test.
 *
 * What this file does instead: take a Comark tree, produce PM JSON with
 * the orchestrator, render it to HTML with `generateHTML`, parse it back
 * with `generateJSON`, and convert the result back to a Comark tree. The
 * tree before and after must match. This exercises:
 *
 *   - Every extension's `renderHTML` (and the implicit `mergeAttributes`
 *     interaction that bit us).
 *   - Every extension's `parseHTML`.
 *   - `htmlAttrSpec`'s end-to-end DOM round-trip — same code path the
 *     editor uses to capture custom data-/aria-/class attributes.
 *   - The full custom-component schema (block and inline) with declared
 *     props plus htmlAttrs leftover.
 *
 * `generateHTML` / `generateJSON` are first-class Tiptap helpers; they
 * use the same PM schema and DOM serializer the live editor does.
 * Running them in happy-dom is enough — no editor mount needed.
 */
...
describe('DOM round-trip — comment node (regression for the data-comark-comment payload-loss bug)', () => {
  // This is the test that would have caught the original bug: the text
  // attr's `renderHTML` emitted `data-comark-comment` with the payload,
  // but the node's `renderHTML` then called
  // `mergeAttributes(HTMLAttributes, { 'data-comark-comment': '' })`,
  // and Tiptap's mergeAttributes uses last-wins for non-class/style
  // attrs, so the empty string clobbered the payload.
...
// ---------------------------------------------------------------------------
// Custom components — block + inline, including the lists.ts fix scenario
// ---------------------------------------------------------------------------
...
describe('DOM round-trip — custom components', () => {
  ...
  // Regression for [lists.ts] — before the fix, list-item bucketing
  // hardcoded `tag === 'br' || tag === 'img'` as the inline set, so a
  // `:badge` inside an `<li>` got treated as a block element and the
  // resulting PM tree was structurally wrong. With the fix, the
  // orchestrator's `parseBlocks` is the single source of truth and
  // any `context: 'inline'` node is recognised correctly.
  it('round-trips an inline component INSIDE a list item (lists.ts regression)', () => {
    const tree: ComarkTree = {
      nodes: [
        [
          'ul',
