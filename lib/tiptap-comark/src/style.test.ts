 ...
 *
 * Tests the auto-injection contract of the operational stylesheet:
 *
 *   - Inserts exactly one `<style data-comark-style>` per document
 *     regardless of how many editors are constructed.
 *   - Honours the `injectStyles: false` opt-out (for hosts shipping
 *     their own complete look, e.g. Nuxt UI).
 *   - Forwards a CSP `injectNonce` to the style tag.
 *   - The `comarkStyle` payload is also exported as a string so
 *     consumers with strict CSP / Shadow DOM / SSR pipelines can do
 *     their own injection — no behavior baked into the auto-inject
 *     should be unreachable from there.
 *
 * These guarantees are the public contract end-users and integrators
 * (Nuxt UI in particular) rely on, so they live in their own test file
 * rather than scattered into the serializer suite.
 */
...
describe('comarkStyle payload', () => {
  ...
  // Regression: shipping the stylesheet without this exclusion was a
  // real bug — `data-comark-component="alert"` lives on the user's
  // Vue NodeView wrapper too (since Tiptap stamps `data-node-view-wrapper`
  // on the same element), and our `::before` "::alert" was rendering
  // on top of the user's component. Every kit selector must skip
  // NodeView-owned hosts so consumers providing a NodeView keep full
