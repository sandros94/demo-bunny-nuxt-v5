/**
 * Top-level integration tests — run real markdown through `comark.parse`,
 * then through the orchestrator's helpers (built from the kit's specs),
 * then back via `comark/render`. The whole-doc round-trip is the test
 * that proves the kit is lossless for everything Comark emits.
 */
