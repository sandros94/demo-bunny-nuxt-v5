/**
 * Orchestrator-level tests. Verifies that `createSerializer`'s dispatch
 * tables wire specs together correctly and that the recursion helpers
 * (`parseBlocks`/`serializeBlocks`/`parseInlines`/`serializeInlines`) call
 * back into the right spec for each child.
 */
