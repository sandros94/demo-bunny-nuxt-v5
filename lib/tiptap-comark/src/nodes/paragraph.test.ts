/**
 * Sidecar tests for `paragraph` — exercises the spec's pure functions
 * (`toComark` / `fromComark`) with stubbed helpers so we don't drag a full
 * editor instance into a unit test.
 */
...
// Helpers built from just this one spec (plus a tiny text-only stub) — for
// testing inline-pass-through. Other tests pull in more specs as needed.
